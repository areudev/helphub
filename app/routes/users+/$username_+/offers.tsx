			...
			<Drawer>
				<div className="flex justify-center md:hidden">
					<DrawerTrigger asChild>
						<Button className="">
							{isOwner ? 'Click to view your offers' : 'Click to view offers'}
						</Button>
					</DrawerTrigger>
				</div>
				<div className="flex">
					<aside className="hidden md:block">
						<div className="p-4">
							<h2 className="mb-4 text-xl font-bold">
								{isOwner ? 'Your' : `${ownerDisplayName}'s`} Offers
							</h2>
							{isOwner ? (
								<Button asChild className="mb-4 w-full">
									<Link to="/announcements">Make an offer</Link>
								</Button>
							) : null}
							{offers.length === 0 ? (
								...
								<ul className="space-y-2">
									{offers.map((offer) => (
										...
										>
											<NavLink
												to={`/users/${owner.username}/offers/${offer.id}`}
												className={({ isActive }) =>
													cn(
														'block rounded p-2',
